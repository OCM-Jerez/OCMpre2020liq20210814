...
export class GraphOrganicoGastoComponent implements AfterViewInit {
  ...
  constructor(
    ...
    this.createData(this.dataGraphGastosService.getOrganico().substring(0, 2))
  ...
  async createData(org: string) {
    this.rowData = await this.avalaibleYearsService.getDataAllYear('Org');
    const datos = this.getObjects(await this.rowData, 'CodOrg', org);
